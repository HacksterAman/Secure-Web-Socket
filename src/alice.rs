...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("[Alice] Using QKD key as PSK for Noise Protocol\n");
    ...
    if let Some(msg) = ws_receiver.next().await {
        match msg? {
            Message::Text(text) if text == "NAME_REQUEST" => {
                ws_sender.send(Message::Text("Alice".to_string())).await?;
            }
    ...
    let noise_session = match perform_noise_handshake_initiator(&mut ws_sender, &mut ws_receiver, &psk).await {
        Ok(session) => session,
        Err(e) => {
            eprintln!("[Alice] Handshake failed: {}", e);
            return Ok(());
        }
    };

    println!("[Alice] Secure channel established. Ready to chat.\n");
            ...
            match msg {
                Ok(Message::Binary(encrypted_data)) => {
                    ...
                    match session.decrypt(&encrypted_data) {
                        ...
                        Err(e) => {
                            eprintln!("[Alice] Decryption failed: {}", e);
                        }
                    }
                }
                Ok(Message::Close(_)) => {
                    println!("[Alice] Server disconnected");
                    break;
                }
                _ => {}
            }
